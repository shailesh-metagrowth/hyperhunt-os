...
| **Hiring Pipeline** | The staged process belonging to a Role: New → CV Shared → Round 1 → Round 2 → Round 3 → Offer → Joined. Canonical set by default on every role; a custom stage can be added case-by-case when a specific mandate needs one. Round 2 and Round 3 appear only if the role's round count enables them. The consultant's primary workspace. |
