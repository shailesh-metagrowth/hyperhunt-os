...
|---|---|---|---|
| **1** | Duplicate data entry | Spine + Magic Upload + event log foundation | A real CV uploaded to a real role produces the full automatic cascade |
...
6. **One-tap interaction log:** calls and WhatsApp messages happen outside the system, so nudges can be wrong ("chase feedback" an hour after the consultant chased it) — and wrong nudges destroy trust in all nudges. Minimal fix: a single-tap "called candidate / chased feedback" event, no typing. Tension: it is technically manual entry. Decide whether one tap is data entry or a heartbeat before Sprint 3.
---
